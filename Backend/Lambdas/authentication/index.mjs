...
const client = jwksClient({
  jwksUri: 'https://cognito-idp.us-east-1.amazonaws.com/us-east-1_8zNNpg8Ps/.well-known/jwks.json'
});
...
async function authenticate(event, callback) {
 ...
 var token = event.authorizationToken
  ...
  return new Promise((resolve, reject) => {
    getKey({ kid }, (err, key) => {
      if (err){
        console.log("REJECTED IN GET KEY")
        reject('Unauthorized');
      } 
      ...
      jsonwebtoken.verify(token, key, { algorithms: ['RS256'] }, (err, decoded) => {
        if (err) {
          console.log(err)
            console.log("REJECTED IN VERIFY")
        ...
        console.log(decodedToken.payload['custom:isLocal'])
        ...
        cognito.adminGetUser(params, (err, data) => {
          if (err) {
            ...
            if(isLocal === "true" && decodedToken.payload['custom:isLocal'] === "true"){
              ...
              console.log(isLocal)
              console.log(decodedToken.payload['custom:isLocal'])
              console.log(isLocal == decodedToken.payload['custom:isLocal'])
              console.log(isLocal === decodedToken.payload['custom:isLocal'])

              console.log("111") 
              console.log(typeof(isLocal))
              console.log(typeof(decodedToken.payload['custom:isLocal']))
